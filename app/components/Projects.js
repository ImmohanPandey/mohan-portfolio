...
export default function Projects() {
  ...
  return (
    <SectionWrapper id="projects">
      <div className="py-20 md:py-32 px-4 sm:px-6 lg:px-12">        <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          viewport={{ once: false }}
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4">            Selected Professional Work
          </h2>

          <p className="text-zinc-500 text-base md:text-lg max-w-3xl">            A collection of products and platforms I have
            built and contributed to across web, mobile,
        ...
        <motion.div
          ...
          className="mt-10 md:mt-16 border border-zinc-800 rounded-3xl p-5 sm:p-6 md:p-10 bg-zinc-950 hover:border-zinc-600 transition-all"        >
          <span className="text-sm text-zinc-500">
            FEATURED PROJECT
          </span>

          <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mt-3 mb-5">            {featuredProject.title}
          </h3>
          ...
          <div className="flex flex-wrap gap-2 mt-6">
            {featuredProject.tech.map((tech) => (
              <span
                key={tech}
                className=" px-4 py-2 rounded-full border border-zinc-800 bg-zinc-900 text-sm "
              >
                {tech}
              </span>
            ))}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mt-8">           {featuredProject.architecture.map((item) => (
            <div
              key={item.title}
              className="p-3 md:p-5 rounded-2xl bg-zinc-900 border border-zinc-800 hover:border-zinc-600 transition-all"
            >
              <p className="text-zinc-500 text-sm">
                {item.title}
              </p>

              <h4 className="mt-2 text-sm md:text-base font-semibold break-words">                {item.value}
              </h4>
            </div>
          ))}
          </div>

          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 mt-8">            {featuredProject.highlights.map(
            (highlight) => (
              <div
                key={highlight}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-10">
          {otherProjects.map((project, index) => (
            <motion.div
              ...
              className="border border-zinc-800 rounded-3xl p-5 md:p-8 bg-zinc-950 hover:border-zinc-600 transition-all duration-300"
            >
              <h3 className="text-xl md:text-2xl font-semibold mb-4">                {project.title}
              </h3>
